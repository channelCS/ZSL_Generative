...
class CLASSIFIER:
    ...
    def __init__(
        ...
        if self.netDec:
            ...
            with torch.no_grad():
                ...
                self.test_unseen_feature = self.compute_dec_out(
                    self.test_unseen_feature, self.input_dim
                )
                self.test_seen_feature = self.compute_dec_out(
                    self.test_seen_feature, self.input_dim
                )
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            ...
            with torch.no_grad():
                acc = self.val(
                    self.test_unseen_feature, self.test_unseen_label, self.unseenclasses
                )
            # print('acc %.4f' % (acc))
            self.model.train()
            if acc > best_acc:
    ...
    def compute_per_class_acc_gzsl(self, test_label, predicted_label, target_classes):
        ...
        for i in target_classes:
            idx = test_label == i
            acc_per_class += (
                torch.sum(test_label[idx] == predicted_label[idx]).item()
                / torch.sum(idx).item()
            )
