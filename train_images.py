...
netDec = model.AttDec(opt,opt["att_size"])
...
input_att = torch.FloatTensor(opt["batch_size"], opt["att_size"]) # att_size class-embedding size
noise = torch.FloatTensor(opt["batch_size"], opt["att_size"])
...
def generate_syn_feature(generator,classes, attribute,num,netF=None,netDec=None):
    ...
    syn_noise = torch.FloatTensor(num, opt["att_size"])
    if cuda:
...
optimizerD         = optim.Adam(netD.parameters(), lr=opt["network"]["gan"]["lr"],betas=(opt.beta1, 0.999))
optimizerG         = optim.Adam(netG.parameters(), lr=opt["network"]["gan"]["lr"],betas=(opt.beta1, 0.999))
optimizerF         = optim.Adam(netF.parameters(), lr=opt["network"]["feedback"]["lr"], betas=(opt.beta1, 0.999))
optimizerDec       = optim.Adam(netDec.parameters(), lr=opt["network"]["decoder"]["lr"], betas=(opt.beta1, 0.999))
...
for epoch in range(0,opt["num_epoch"]):
    for loop in range(0,opt["feedback_loop"]):
        for i in range(0, data.ntrain, opt["batch_size"]):
            ...
            for iter_d in range(opt["critic_iter"]):
                ...
                criticD_real = opt["gamma_d"]*criticD_real.mean()
                ...
                criticD_fake = opt["gamma_d"]*criticD_fake.mean()
                criticD_fake.backward(one)
                # gradient penalty
                gradient_penalty = opt["gamma_d"]*calc_gradient_penalty(netD, input_res, fake.data, input_att)
            ...
            if loop == 1:
                optimizerF.step()
            if opt.recons_weight > 0: # not train decoder at feedback time
                optimizerDec.step() 
        
    print('[%d/%d]  Loss_D: %.4f Loss_G: %.4f, Wasserstein_dist:%.4f, vae_loss_seen:%.4f'% (epoch, opt.num_epoch, D_cost.data[0], G_cost.data[0], Wasserstein_D.data[0],vae_loss_seen.data[0]),end=" ")
    netG.eval()
    netDec.eval()
    netF.eval()
    ...
    if opt["network"]["classifier"]["gzsl"]:   
        ...
        gzsl_cls = classifier.CLASSIFIER(train_X, train_Y, data, nclass, cuda, opt["network"]["classifier"]["lr"], 0.5, \
                25, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt["att_size"], dec_hidden_size=4096)
    ...
    zsl_cls = classifier.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses), \
                    data, data.unseenclasses.size(0), opt.cuda, opt["network"]["classifier"]["lr"], 0.5, 25, opt["syn_num"], \
                    generalized=False, netDec=netDec, dec_size=opt["att_size"], dec_hidden_size=4096)
    acc = zsl_cls.acc
    if best_zsl_acc < acc:
        best_zsl_acc = acc
...
if opt["network"]["classifier"]["gzsl"]:
    print('Dataset', opt["dataset"]["name"])
